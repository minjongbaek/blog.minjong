...
`CommonJS`와 webpack의 등장으로 웹 프론트엔드 생태계는 폭발적인 성장을 할 수 있었다. 그러나, `CommonJS`의 런타임에 모듈을 동기적으로 불러오는 방식 때문에 트리 셰이킹과 같은 최적화 작업에 한계가 있었다. 예를들어 lodash 에서 일부 함수만 사용하는 경우 webpack은 어떤 함수를 사용할지 알지 못하기 때문에 lodash 전체를 번들에 포함시켜버렸다.
